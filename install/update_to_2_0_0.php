...
function update_to_2_0_0() :String {
   ...
   if ($DB->tableExists('glpi_plugin_impacts_impacts')) {
      ...
      foreach ($items as $item) {
         ...
         $item['itemtype_source'] = $item['itemtype_1'];
         $item['items_id_source'] = $item['items_id_1'];
         $item['itemtype_impacted'] = $item['itemtype_2'];
         $item['items_id_impacted'] = $item['items_id_2'];
         unset($item['itemtype_1']);
         unset($item['items_id_1']);
         unset($item['itemtype_2']);
         unset($item['items_id_2']);
      ...
      $itemtype = new PluginImpactsItemtype;
      foreach ($used_types as $tp) {
         $img = $tp;
         if ($tp == 'Appliance') {
            $img = 'PluginAppliancesAppliance';
         }
         $itemtype->add(['name' => $tp, 'url_path_pics' => "plugins/impacts/pics/$img.png"]);
      }
